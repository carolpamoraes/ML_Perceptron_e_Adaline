...
for passo=[10^-6]
    ...
    %TESTE
    x_teste=[ones(1,q_teste);dataset_mix((q_tr+1):total,2)'];
    y_d_teste=dataset_regre((q_tr+1):total,1);
    ...
    for ep=1:epoca
            ...
            for it=1:q_teste
                ...
                erro=y_d_teste(it,:)-y_teste;

                w=w+passo*erro*x_teste(:,it);    
...
hold on
x_F=[0:total];
Fron_tr=Modelo_linear_tr(2,1)*x_F+Modelo_linear_tr(1,1);
plot(x_F,Fron_tr,'--','Color',[0.3010 0.7450 0.9330],'LineWidth',2)
